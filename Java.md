...
- 因为String不可变，只会指向新的地址，所以可以实现字符串常量池，节约内存空间，提高效率（创建String a = "abc"时，会先检查字符串常量池中有无"abc"，若由则直接指向字符串常量池中的变量，如果没有就在字符串常量池中新建字符串对象）。
...
![spring overview](https://docs.spring.io/spring-framework/docs/5.0.0.RC3/spring-framework-reference/images/spring-overview.png)

#### 1.IOC（控制反转）

> 概念

- 控制反转：借助于“第三方”实现具有依赖关系的对象之间的解耦。
- 依赖注入：将实例变量传入到另一个对象当中去（依赖注入是控制反转思想的实现方式）。
- 在Spring中“第三方”指的就是IOC容器，例如：Class A要调用Class B，常规做法要在A中new出B对象，但通过IOC将创建B对象的过程交给了**IOC容器**，然后直接**注入**A中，而不需要在Class A中进行。

> 优点

- 解耦：将**被调用类**作为参数传入**调用类**，实现了上层类对下层类的控制。当对底层对象进行修改时，new对象的方式可能需要修改上层类中的代码，耦合性很高，不利于维护，而依赖注入直接将底层对象注入到上层类中，不需要修改上层类的代码。

  https://www.zhihu.com/question/23277575

- 减少重复代码：假设B中调用了C，C中调用了D，如果没有IOC容器，我们需要先new D然后将D传入C再将C传入B，再将B传入A，这样需要写大量new。有了IOC容器这一过程直接交给IOC容器而不用自己手写。

> 底层实现

- 反射，工厂模式

  https://juejin.cn/post/7004462914761392135
...
​		运行时动态地将代码切入到类的指定方法，指定位置上的编程思想。主要用于将那些与业务无关，却为业务各模块所共同调用的逻辑封装起		来，如：事务处理，日志管理，权限控制等。

> 优点

​		增强原有方法，减少系统的重复代码，降低模块间的耦合度，有利于扩展与维护。
...
Spring 中的事务实现从原理上说比较简单，通过AOP在方法执行前后增加数据库事务的操作。
